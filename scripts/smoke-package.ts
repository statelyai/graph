  ...
  const exportChecks: Record<string, ExportCheck> = {
    '.': {
      ...
      runtime: [
        ...
        `assert.equal(typeof $MOD.genBFS, 'function');`,
        `assert.equal(typeof $MOD.updateGraphWithPatches, 'function');`,
        `assert.equal(typeof $MOD.getFlattenedGraph, 'function');`,
        `assert.equal(typeof $MOD.genWalkSteps, 'function');`,
      ...
      types: [
        ...
        `const nodes = [...$MOD.genBFS(graph, 'a')];`,
        `$MOD.updateGraphWithPatches(graph, []);`,
        `$MOD.getFlattenedGraph(graph);`,
        `[...$MOD.genWalkSteps($MOD.genRandomWalk(graph, { from: 'a' }), 1)];`,
        `nodes[0]?.id;`,
        `graph.nodes[0]?.id;`,
      ],
    },
    './algorithms': {
      ...
      runtime: [
        ...
        `assert.equal(typeof $MOD.genBFS, 'function');`,
        `assert.equal(typeof $MOD.genDFS, 'function');`,
        `assert.equal(typeof $MOD.getJoinedPath, 'function');`,
      ...
      types: [
        ...
        `const visited = [...$MOD.genBFS(graph, 'a')];`,
