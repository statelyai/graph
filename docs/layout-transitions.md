...
Layouts are plain graphs, so `getPatches` / `applyPatches` (from `@statelyai/graph`) work on them directly. A patch set between two layouts tells you exactly which nodes moved — e.g. to skip tweening unmoved nodes:
...
And because patches are first-class data, undo/redo falls out for free: keep the patch sets (or use `getDiff` + `invertDiff`) and `applyPatches` to step layout history in either direction.
